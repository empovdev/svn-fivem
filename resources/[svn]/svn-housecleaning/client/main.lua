...
RegisterNuiCallback('close', function()
    ...
    TriggerServerEvent("svn-housecleaning:leftSquad")
    UIOpen = false
end)

RegisterNuiCallback('getPlayerData', function(_, cb)
    TriggerServerCallback('svn-housecleaning:getPlayerData', function(data)
        PlayerData = data
...
RegisterNuiCallback('toggleInvite', function(val, _)
    TriggerServerCallback('svn-housecleaning:toggleInvite', function(cb)
        if cb then
...
RegisterNuiCallback('sendInvite', function(id, cbb)
    ...
    TriggerServerCallback('svn-housecleaning:sendInvite', function(cb)
        cbb(cb)
        if cb then
...
RegisterNuiCallback('startMission', function(data, cb)
    ...
    if data.members < (mission.MinMembers - 1) then
        ...
        TriggerServerCallback("svn-housecleaning:startMission", function(cbb)
            if cbb then
                SetNuiFocus(false, false)
...
RegisterNuiCallback('kickPlayer', function(license)
    TriggerServerEvent("svn-housecleaning:kick", license)
end)

RegisterNuiCallback('changeImage', function(image, cb)
    cb("ok")
    TriggerServerEvent("svn-housecleaning:changeImage", image)
    TriggerServerCallback("svn-housecleaning:createSquad", function (cb)
        if cb then
            UIOpen = true
...
RegisterNetEvent("svn-housecleaning:closeMenu", function()
    SetNuiFocus(false, false)
    UIOpen = false
    SendNUIMessage({
        action = "closeMenu",
...
RegisterNetEvent("svn-housecleaning:startMission", function (mission, positions)
    SetNuiFocus(false, false)
    UIOpen = false
    SendNUIMessage({
        action = "closeMenu",
...
RegisterNetEvent("svn-housecleaning:receiveInvite", function (inviteId)
...
RegisterNetEvent("svn-housecleaning:refreshSquad", function (squad, isLeader)
    SendNUIMessage({
...
RegisterNetEvent("svn-housecleaning:deleteSquad", function ()
...
RegisterNetEvent("svn-housecleaning:fetchPositions", function(pos)
                ...
                if dist < 1.0 then
                    if HasTrash then
                        ...
                        if IsControlJustPressed(0, 38) then
                            ...
                            Wait(1000)
                            Mission.EnteredHouse = false
                            TriggerServerEvent("svn-housecleaning:leaveHouse")
                            DoScreenFadeIn(500)
                        end
                    end
                end
            ...
            if closestPosition then
                ...
                if dist < 1.0 then
                    ...
                    if IsControlJustPressed(0, 38) then
                        ...
                        TriggerServerEvent("svn-housecleaning:fetchPositions", closestPosition.coords)
                    end
                end
            end
        else
    ...
    if not id then
        ...
        for _, invite in pairs(Invites) do
            if invite.id == id then
                table.remove(Invites, _)
                TriggerServerCallback("svn-housecleaning:acceptInvite", function (cb)
                    if cb then
                        UIOpen = true
                        SetNuiFocus(true, true)
                        SendNUIMessage({
