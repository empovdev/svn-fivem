...
RegisterServerCallback('svn-housecleaning:getPlayerData', function (src, cb)
...
RegisterServerCallback('svn-housecleaning:toggleInvite', function (src, cb, val)
    local license = _GetPlayerLicense(src)
    local file = LoadResourceFile(GetCurrentResourceName(), "database.json")
    local data = json.decode(file)
...
RegisterServerCallback('svn-housecleaning:createSquad', function (src, cb)
    local license = _GetPlayerLicense(src)
...
RegisterServerCallback('svn-housecleaning:sendInvite', function (src, cb, id)
    local license = _GetPlayerLicense(src)
    local file = LoadResourceFile(GetCurrentResourceName(), "database.json")
    local data = json.decode(file)
    ...
    if Squads[license] then
        ...
        TriggerClientEvent('svn-housecleaning:receiveInvite', id, license)
    else
        cb(false)
    end
end)

RegisterServerCallback("svn-housecleaning:acceptInvite", function (src, cb, squad)
    local license = _GetPlayerLicense(src)
    local file = LoadResourceFile(GetCurrentResourceName(), "database.json")
    local data = json.decode(file)
    ...
    if Squads[squad] then
        ...
        for _, member in pairs(Squads[squad].members) do
            if not member.license == license then
                TriggerClientEvent('svn-housecleaning:refreshSquad', member.source, Squads[squad], false)
            end
        end

        TriggerClientEvent('svn-housecleaning:refreshSquad', Squads[squad].leaderSrc, Squads[squad], true)
    else
        cb(false)
    end
end)

RegisterServerCallback('svn-housecleaning:startMission', function(src, cb, mission)
    local license = _GetPlayerLicense(src)
    ...
    if Squads[license] then
        TriggerClientEvent('svn-housecleaning:startMission', Squads[license].leaderSrc, mission, selectedPositions)

        for _, member in pairs(Squads[license].members) do
            TriggerClientEvent('svn-housecleaning:startMission', member.source, mission, selectedPositions)
        end
    end
end)

RegisterServerEvent('svn-housecleaning:leftSquad', function (src)
    local src = src or source
    local license = _GetPlayerLicense(src)

    if Squads[license] then
        for _, member in pairs(Squads[license].members) do
            TriggerClientEvent('svn-housecleaning:deleteSquad', member.source)
        end
        ...
        for _, squad in pairs(Squads) do
            for _, member in pairs(squad.members) do
                if member.license == license then
                    ...
                    TriggerClientEvent('svn-housecleaning:refreshSquad', member.source, squad, false)
                    TriggerClientEvent('svn-housecleaning:refreshSquad', squad.leaderSrc, squad, true)
                    break
                end
            end
        end
...
RegisterServerEvent('svn-housecleaning:enterHouse', function ()
    local src = source
    local license = _GetPlayerLicense(src)
...
RegisterServerEvent('svn-housecleaning:leaveHouse', function ()
    local src = source
    local license = _GetPlayerLicense(src)
...
RegisterServerEvent('svn-housecleaning:fetchPositions', function (pos)
    local src = source
    local license = _GetPlayerLicense(src)

    if Squads[license] then
        TriggerClientEvent('svn-housecleaning:fetchPositions', Squads[license].leaderSrc, pos)

        for _, member in pairs(Squads[license].members) do
            TriggerClientEvent('svn-housecleaning:fetchPositions', member.source, pos)
        end
    else
        local squad = GetPlayerSquad(license)

        if squad then
            TriggerClientEvent('svn-housecleaning:fetchPositions', squad.leaderSrc, pos)
...
RegisterServerEvent('svn-housecleaning:finishMission', function ()
    local src = source
    local license = _GetPlayerLicense(src)
...
RegisterServerEvent("svn-housecleaning:kick", function(license)
    for _, squad in pairs(Squads) do
        for _, member in pairs(squad.members) do
            if member.license == license then
                TriggerClientEvent('svn-housecleaning:closeMenu', member.source)
                table.remove(squad.members, _)

                TriggerClientEvent('svn-housecleaning:refreshSquad', squad.leaderSrc, squad, true)
                
                for _, memberr in pairs(squad.members) do
                    TriggerClientEvent('svn-housecleaning:refreshSquad', memberr.source, squad, false)
                end
...
RegisterServerEvent("svn-housecleaning:changeImage", function(img)
    local src = source
    local license = _GetPlayerLicense(src)
    local file = LoadResourceFile(GetCurrentResourceName(), "database.json")
    local data = json.decode(file)
...
AddEventHandler('playerDropped', function ()
    local src = source
    TriggerEvent('svn-housecleaning:leftSquad', src)
end)