...
Citizen.CreateThread(function()
    ...
    for k, v in pairs(Config.VehicleShops) do
        ...
        if v.Management.Enable then
            exports['svn-textui-2']:create3DTextUI("svn-vehicleshop-vehshop-" .. k, {
                ...
                triggerData = {
                    triggerName = "svn-vehicleshop:openSellingMenu:client",
                    args = {vehshopId = k, open = true}
                }
            })
        end
    end
...
RegisterNetEvent('svn-vehicleshop:openSellingMenu:client', function(data)
    sellMenuActive = data.open
    if sellMenuActive then
        local showAddStockBtn = false
        TriggerCallback('svn-vehicleshop:checkIsPlayerHasPerm:server', function(hasPerm)
            if hasPerm then
        ...
        if Config.VehicleShops[data.vehshopId].EnableStocks then
            ...
            TriggerCallback('svn-vehicleshop:getVehStock:server', function(stockData)
                if stockData ~= 0 then
            ...
            Citizen.Wait(1000)
            SendNUIMessage({action = "openSellMenu", dealershipId = data.vehshopId, vehicles = currentMenuVehs, vehicles2 = currentMenuVehs, showAddStockBtn = showAddStockBtn})
        else
...
--     Initialized = true
--     Citizen.CreateThread(function()
--         TriggerCallback('svn-vehicleshop:getShowroomData:server', function(showroomData)
...
--                     exports['svn-textui-2']:create3DTextUI("svn-vehicleshop-showroom-vehicle-" .. model .. "-spotId-" .. v.spotId, {
...
--                         theme = "green", 
--                         triggerData = {
--                             triggerName = "svn-vehicleshop:askAnEmployee:client",
--                             args = {}
--                         }
--                     })
--                 end
...
RegisterNetEvent('svn-vehicleshop:askAnEmployee:client', function()
...
function openDealership(state, id)
    ...
    if menuActive then
        ...
        Citizen.Wait(350)
        SendNUIMessage({action = "openDealership", customImg = Config.UseCustomImages, dealershipId = id, managementState = false, vehicles = vehiclesT, categories = categoriesData, testDriveTime = Config.TestDriveTime})
    else
...
RegisterNetEvent('pa-dealership:close:client', function()
...
RegisterNUICallback('callback', function(data)
    ...
    elseif data.action == "close" then
        ...
        elseif data.type == "showroomEdit" then
            ...
            TriggerServerEvent('svn-vehicleshop:updateShowroomVehicles:server', data.dealershipId, Config.VehicleShops[data.dealershipId].ShowroomVehicles)
        elseif data.type == "sellMenu" then
            TriggerEvent('svn-vehicleshop:openSellingMenu:client', {open = false})
        else
            SetNuiFocus(false, false)
        end
        if data.test == true and not testDriveState then
            if IsSpawnPointClear(vector3(currentCoords.testVehCoords.x, currentCoords.testVehCoords.y, currentCoords.testVehCoords.z), 2.5) then
                ...
                TriggerServerEvent('svn-vehicleshop:startTest:server', currentTestVehicleNetId)
    ...
    elseif data.action == "updateShowroomVehicle" then
        ...
        --TriggerServerEvent('svn-vehicleshop:deleteVehicleShowroom:server', data.dealershipId, data.spotId, data.model, props)
        TriggerServerEvent('svn-vehicleshop:deleteVehicleShowroom:server', data.dealershipId, data.spotId, data.model)
    ...
    elseif data.action == "createTextsOnPlayers" then
        ...
        if next(nearbyPlayers) ~= nil and next(nearbyPlayers) then
            ...
            for _, id in pairs(nearbyPlayers) do
                exports['svn-textui-2']:create3DTextUIOnPlayers("svn-vehicleshop-vehshop-players-" .. id, {
                    id = id,
                    displayDist = 5.0,
                    interactDist = 1.3,
                    ...
                    theme = "green", -- or red
                    triggerData = {
                        triggerName = "svn-vehicleshop:sendSellRequest:client",
            ...
            if next(nearbyPlayers) ~= nil and next(nearbyPlayers) then
                for _, id in pairs(nearbyPlayers) do
                    exports['svn-textui-2']:delete3DTextUIOnPlayers("svn-vehicleshop-vehshop-players-" .. id)
                end
            end
        else
    ...
    elseif data.action == "confirmPurchase" then
        TriggerServerEvent('svn-vehicleshop:buyVehicle:server', data.paymentType, data.model, Vehicles2[data.model].price, data.dealershipId, data.sender, Config.VehicleShops[data.dealershipId].Management.Job)
    elseif data.action == "addStockForAllVehicles" then
        ...
        TriggerServerEvent('svn-vehicleshop:updateDealershipStockData:server', data.dealershipId, stocks)
        taskActive = false
...
RegisterNetEvent('svn-vehicleshop:sendSellRequest:client', function(data)
    -- Delete All
    if next(nearbyPlayers) ~= nil and next(nearbyPlayers) then
        for _, id in pairs(nearbyPlayers) do
            exports['svn-textui-2']:delete3DTextUIOnPlayers("svn-vehicleshop-vehshop-players-" .. id)
    ...
    -- Add To Sender
    TriggerServerEvent('svn-vehicleshop:sendRequestText:server', data.sender, data.target, data.price, data.model, data.dealershipId)
    Citizen.Wait(500)
    taskActive = false
end)

RegisterNetEvent('svn-vehicleshop:sendRequestText:client', function(sender, price, model, dealershipId)
    exports['svn-textui-2']:create3DTextUIOnPlayers("svn-vehicleshop-vehshop-players-confirm-purchase-" .. sender, {
        id = sender,
        displayDist = 5.0,
        interactDist = 1.3,
        enableKeyClick = true,
    ...
    taskActive = false
    Notify("Request timed out.", 7500, "error")
    exports['svn-textui-2']:delete3DTextUIOnPlayers("svn-vehicleshop-vehshop-players-confirm-purchase-" .. sender)
end)

RegisterNetEvent('svn-vehicleshop:acceptPayment:client', function(data)
    Notify("Request accepted.", 7500, "success")
    exports['svn-textui-2']:delete3DTextUIOnPlayers("svn-vehicleshop-vehshop-players-confirm-purchase-" .. data.sender)
    TriggerServerEvent('svn-vehicleshop:buyVehicle:server', "bank", data.model, tonumber(data.price), data.dealershipId, data.sender, Config.VehicleShops[data.dealershipId].Management.Job)
end)

RegisterNetEvent('svn-vehicleshop:declinePayment:client', function(data)
    Notify("Request declined.", 7500, "error")
    exports['svn-textui-2']:delete3DTextUIOnPlayers("svn-vehicleshop-vehshop-players-confirm-purchase-" .. data.sender)
    TriggerServerEvent('svn-vehicleshop:declinePayment:server', data.sender)
end)
...
AddEventHandler('onResourceStop', function(resource)
    if GetCurrentResourceName() == resource then
        ...
        for k, v in pairs(Config.VehicleShops) do
            ...
            exports['svn-textui-2']:delete3DTextUI("svn-vehicleshop-vehshop-" .. k)
        end
    end
end)

AddEventHandler('pa-deathscreen:openUI', function()
    if menuActive then
        openDealership(false)
    end
end)

AddEventHandler('pa-deathscreen:die', function()
    if menuActive then
        openDealership(false)
    end
end)
...
RegisterNetEvent('svn-vehicleshop:buyVehicle:client', function(model, plate, dealershipId)
    DoScreenFadeOut(500)
    openDealership(false)
    Citizen.Wait(1000)
...
RegisterNetEvent('svn-vehicleshop:finishTest:client', function()
    testDriveState = false
...
AddEventHandler('gameEventTriggered', function(event, data)
    if event == "CEventNetworkVehicleUndrivable" then
        ...
        if netId == currentTestVehicleNetId then
            TriggerEvent('svn-vehicleshop:finishTest:client')
            TriggerServerEvent('svn-vehicleshop:finishTest:server', netId)
        end
    end
                ...
                for k, v in pairs(Config.VehicleShops) do
                    ...
                    if dist <= 3 then
                        function currentShow()
                            exports["svn-textui-2"]:displayTextUI("Give Back Test Vehicle", "E")
                            showTextUI = true
                        end
                        function currentHide()
                            showTextUI = false
                            exports["svn-textui-2"]:hideTextUI()
                        end
				...
				if closestDeliveryPoints.distance < closestDeliveryPoints.maxDist then
					if IsControlJustReleased(0, 38) then
                        ...
                        if VehToNet(veh) == VehToNet(currentTestVehicle) then
                            testDriveState = false
                            SendNUIMessage({action = "stopTestTimer"})
                            TriggerServerEvent('svn-vehicleshop:finishTest:server', VehToNet(currentTestVehicle))
...
RegisterNetEvent('svn-vehicleshop:deleteVehicle:client', function(netId)
...
--RegisterNetEvent('svn-vehicleshop:deleteVehicleShowroom:client', function(dealershipId, spotId, newModel, props)
RegisterNetEvent('svn-vehicleshop:deleteVehicleShowroom:client', function(dealershipId, spotId, newModel, createNew)
    -- Delete Old
