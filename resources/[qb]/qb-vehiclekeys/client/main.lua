...
RegisterCommand('engine', function()
    ...
    if vehicle == 0 or GetPedInVehicleSeat(vehicle, -1) ~= PlayerPedId() and not GetVehicleClass(vehicle, 18) then return end
    if GetIsVehicleEngineRunning(vehicle) then
        QBCore.Functions.Notify('Engine Off', 5000, 'error', true)
    else
        QBCore.Functions.Notify('Engine Started', 5000, 'success', true)
...
RegisterNetEvent('qb-vehiclekeys:client:GiveKeys', function(id)
    ...
    if targetVehicle then
        ...
        if HasKeys(targetPlate) then
            ...
            QBCore.Functions.Notify(Lang:t("notify.ydhk"), 5000, 'error', true)
        end
    end
end)
        ...
        if not isBlacklistedVehicle(veh) then
            if HasKeys(QBCore.Functions.GetPlate(veh)) or AreKeysJobShared(veh) then
                ...
                if vehLockStatus == 1 then
                    ...
                    QBCore.Functions.Notify(Lang:t("notify.vlock"), 5000, 'success', true)

                else
                    TriggerServerEvent('qb-vehiclekeys:server:setVehLockState', NetworkGetNetworkIdFromEntity(veh), 1)
                    QBCore.Functions.Notify(Lang:t("notify.vunlock"), 5000, 'success', true)
                end
...
function GiveKeys(id, plate)
    ...
    if distance < 1.5 and distance > 0.0 then
        ...
        QBCore.Functions.Notify(Lang:t("notify.nonear"), 5000, 'error', true)
    end
end
function GetKeys()
...
function GetVehicle()
    ...
    while vehicle == 0 do
        ...
        if #(pos - GetEntityCoords(vehicle)) > 8 then
            QBCore.Functions.Notify(Lang:t("notify.vehclose"), 5000, 'error', true)
            return
        end
    end
...
function LockpickFinishCallback(success)
    ...
    if success then
        ...
        if GetPedInVehicleSeat(vehicle, -1) == PlayerPedId() then
            ...
            QBCore.Functions.Notify(Lang:t("notify.vlockpick"), 5000, 'success', true)
...
function Hotwire(vehicle, plate)
    ...
    QBCore.Functions.Progressbar("hotwire_vehicle", Lang:t("progress.hskeys"), hotwireTime, false, true, {
        ...
        if (math.random() <= Config.HotwireChance) then
            ...
            QBCore.Functions.Notify(Lang:t("notify.fvlockpick"), 5000, 'error', true)
        end
    ...
    QBCore.Functions.Progressbar("rob_keys", Lang:t("progress.acjack"), Config.CarjackingTime, false, true, {}, {}, {}, {}, function()
        ...
        if hasWeapon and isCarjacking then
            ...
            if math.random() <= carjackChance then
                ...
                QBCore.Functions.Notify(Lang:t("notify.cjackfail"), 5000, 'error', true)
